% run_gain_records
%MODIFY to include Bfiled applied 
list=dir([pwd '/*_out.mat']);
for j=1:length(list)
    folder=list(j).name(1:end-8);
    
    load(list(j).name, 'record');
...
save([list(j).name(1:end-8) '_gain'], 'folder','tim','ro1','re','rm','torinn','gginf', 'g','bext','brad','bphi','bdip','bcua');
clear tim ro1 re rm torinn gginf g brad bphi bdip bcua
