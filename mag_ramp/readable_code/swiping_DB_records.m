...
for i = 1:L
    ...
    ro0 = results{1,i}{1,2}(1);
    fi0 = results{1,i}{1,2}(2);
    fo0 = results{1,i}{1,2}(3);
    
    if (abs(ro0) < 0.01 || abs(ro0+1) ) && abs(fo0 + 1.5) < 0.3
        
        if 1  % put an additional criteria here if you want
            mag_data = results{1,i}{1,3};   % saving I(t) 
            Lm = length(mag_data);          % length of mag data
            gap = 50*10;                    % minimum amount of points to satisfy criteria of 30 seconds of stable mag field x10 (sampling rate)
            gap = min(Lm,gap);              % making sure gap is not longer than size of the data

            for j = 1:10:Lm-gap             % checking only every second

                goal_I = 0; % the mag field we want to find in the record
                if  mean(abs(mag_data(j:j+gap) - goal_I)) < 3  % if average error is less than one => pass criteria

                    days = [days;  results{1,i}{1}];            % adding the day of the run to our list
                    rossbys = [rossbys, results{1,i}{1,2}(1)];  % adding Ro
                    out_freq = [out_freq, results{1,i}{1,2}(3)];
...
figure
scatter(1:size(rossbys,2), sort(out_freq(:)),'.')
title('Found f_{out}');
xlabel('Counter')
